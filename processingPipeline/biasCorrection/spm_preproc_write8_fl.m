function [cls,M1] = spm_preproc_write8_fl(res,tc,bf,df,mrf,cleanup,bb,vx, odir, biasfwhm, biasreg, samp)
...
for n=1:N
    ...
    if bf(n,2)
        chan(n).Nc      = nifti;
        chan(n).Nc.dat  = file_array(fullfile(pth1, [nam1 strrep(['_fwhm-' num2str(biasfwhm) '_reg-' num2str(biasreg) '_samp-' num2str(samp)], '.', '') '.nii']), ...
                                     res.image(n).dim(1:3),...
